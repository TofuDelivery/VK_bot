...
public class VKManager {
    ...
    public void sendMessage(String msg, int peerId){
        ...
        try {
            vkCore.getVk().messages().send(vkCore.getActor()).peerId(peerId).message(msg).randomId(ThreadLocalRandom.current().nextInt()).execute();
        }catch (ApiException | ClientException e){
            e.printStackTrace();
        }
    }
    ...
    public void sendPicture(String photoId, int peerId){
        try {
            vkCore.getVk().messages().send(vkCore.getActor()).peerId(peerId).attachment(photoId).execute(); //TODO: Поменял формат attachment, из-за того как она работает в 1.0.6 версии api
        } catch (ApiException | ClientException e) {
            e.printStackTrace();
        }
    }
    ...
    public void sendKeyBoard(String msg, int peerId, ArrayList<String> buttonTexts){
        ...
        try {
            vkCore.getVk().messages().send(vkCore.getActor()).peerId(peerId).message(msg).randomId(ThreadLocalRandom.current().nextInt()).unsafeParam("keyboard", keyboard).execute();
    ...
    public void sendSticker(int peerId){
        try {
            vkCore.getVk().messages().send(vkCore.getActor()).peerId(peerId).stickerId(141).randomId(ThreadLocalRandom.current().nextInt()).execute();
        } catch (ApiException | ClientException e) {
            e.printStackTrace();
        }
